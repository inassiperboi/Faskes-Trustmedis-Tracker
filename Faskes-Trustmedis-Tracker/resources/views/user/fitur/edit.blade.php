...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            ...
            <form action="{{ route('fitur.update', $fitur->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('no_assessment', $fitur->no_assessment) }}" 
                           required>
                    @error('no_assessment')
                ...
                <div>
                    ...
                    <textarea name="judul" 
                              ...
                              rows="3"
                              class="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              required>{{ old('judul', $fitur->judul) }}</textarea>
                    @error('judul')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" 
                               name="target_uat" 
                               class="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               value="{{ old('target_uat', $fitur->target_uat?->format('Y-m-d')) }}">
                        @error('target_uat')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               ...
                               class="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               value="{{ old('target_due_date', $fitur->target_due_date?->format('Y-m-d')) }}">
